  ...
  module Session
    class RedisRepository < Repository
      ...
      def create_agent(session_id, session_agent)
        ...
        agents = data["agents"]
        raise Rubyrana::SessionError, "Agent #{session_agent.agent_id} already exists in session #{session_id}" if agents.key?(session_agent.agent_id)
      ...
      def update_agent(session_id, session_agent)
        data = ensure_data(session_id)
        agents = data["agents"]
        raise Rubyrana::SessionError, "Agent #{session_agent.agent_id} does not exist in session #{session_id}" unless agents.key?(session_agent.agent_id)
      ...
      def create_message(session_id, agent_id, session_message)
        ...
        raise Rubyrana::SessionError, "Message #{session_message.message_id} already exists in agent #{agent_id} in session #{session_id}" if messages.key?(session_message.message_id)
      ...
      def update_message(session_id, agent_id, session_message)
        ...
        raise Rubyrana::SessionError, "Message #{session_message.message_id} does not exist in session #{session_id}" unless messages.key?(session_message.message_id)
      ...
      def update_multi_agent(session_id, multi_agent)
        data = ensure_data(session_id)
        raise Rubyrana::SessionError, "MultiAgent #{multi_agent.id} does not exist in session #{session_id}" unless data["multi_agents"].key?(multi_agent.id)
