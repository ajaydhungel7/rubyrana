...
class FixturesTest < Minitest::Test
  ...
  def test_mock_hook_provider_filters_events
    ...
    provider = Rubyrana::TestFixtures::MockHookProvider.new([:before_request, :on_tool_call])
    ...
    assert_equal [:before_request, :on_tool_call], provider.event_types_received
  end

  def test_mock_multiagent_hook_provider_records
    provider = Rubyrana::TestFixtures::MockMultiagentHookProvider.new([:before_node_call, :after_node_call])
    provider.before_node_call({ node: "a" })
    provider.after_node_call({ node: "a" })
    provider.after_multiagent_invocation({})

    assert_equal [:before_node_call, :after_node_call], provider.event_types_received
  end
