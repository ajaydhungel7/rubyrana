...
module Rubyrana
  module TestFixtures
    class SessionException < Rubyrana::Error; end
    ...
    class MockSessionRepository
      ...
      def create_session(session)
        ...
        raise SessionException, "Session #{session_id} already exists" if @sessions.key?(session_id)
      ...
      def list_messages(session_id, agent_id, limit: nil, offset: 0)
        ...
        sorted = messages.keys.sort.map { |key| messages[key] }
      ...
      def ensure_session!(session_id)
        raise SessionException, "Session #{session_id} does not exist" unless @sessions.key?(session_id)
      end
